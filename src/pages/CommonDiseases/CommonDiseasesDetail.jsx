...
const CommonDiseasesDetail = () => {
      ...
      dispatch(getDiseaseDetail(diseaseId)).then((res) => {
        ...
        if (res.payload) {
          const updatedFormData = {
            ...
            name: res.payload.name,
            description: res.payload.description,
            type: res.payload.type || 0,
            foodModifyPercent: res.payload.foodModifyPercent,
            saltModifyPercent: res.payload.saltModifyPercent,
            image: res.payload.image || "",
            medicineIds: res.payload.medicineIds || [],
          };
          setFormData(updatedFormData);
          setPreviewImage(res.payload.image);
  ...
  const handleSave = async () => {
    try {
      const values = await form.validateFields();
      // Construct the updatedData object with all required fields for the API
      let updatedData = {
        diseaseId: diseaseId,
        name: values.name || disease.name,
        description: values.description || disease.description,
        type: disease.type || 0, // Ensure type is included
        foodModifyPercent:
          values.foodModifyPercent || disease.foodModifyPercent,
        saltModifyPercent:
          values.saltModifyPercent || disease.saltModifyPercent,
        image: disease.image || "", // Default to current image
        medicineIds: disease.medicineIds || [], // Ensure medicineIds is included
      };

      if (newImageFile) {
        const imageUrl = await handleImageUpload(newImageFile);
        if (imageUrl) {
          updatedData.image = imageUrl; // Update the image field with the new imageUrl
        } else {
            ...
            <div>
              ...
              {disease?.medicines && disease.medicines.length > 0 ? (
                <List
                  dataSource={disease.medicines}
                  renderItem={(medicine) => {
                    const product = productsById[medicine.productId];
                    return (
                      <List.Item className="pl-2">
                        <div className="flex items-start">
                          <div className="mr-4">
