...
function ShopTable({ dataSource }) {
  ...
  const handleDelete = (record) => {
    console.log("Delete clicked for shop ID:", record.shopId);
    setLoading(true);
    dispatch(deleteShop(record.shopId))
      .then(() => {
        notification.success({
  ...
  const handleViewDetails = (shopId, userId) => {
    navigate(`/admin/shop-details/${shopId}/${userId}`);
  };
        ...
        return (
          <div style={{ display: "flex", gap: "8px" }}>
            ...
            <Button
              style={{ width: "32px", height: "32px" }}
              type="primary"
              onClick={() => handleViewDetails(record.shopId, record.userId)}
            >
              <EyeOutlined />
            </Button>
          </div>
